...
perl -i -p -e "s/^#(apache2_vhosts_proxy_pass):.*/\1: \"\/   http:\/\/localhost:{{ crowd_connector_port }}\/\"/g" $TMP_VARS
perl -i -p -e "s/^#(apache2_vhosts_proxy_pass_reverse):.*/\1: \"\/   http:\/\/localhost:{{ crowd_connector_port }}\/\"/g" $TMP_VARS
...
perl -i -p -e "s/^#(apache2_vhosts_server_name):.*/\1: \"{{ crowd_proxy_name }}\"/g" $TMP_VARS
...
perl -i -p -e "s/^#(crowd_catalina):.*/\1: \"\/usr\/share\/crowd\"/g" $TMP_VARS
perl -i -p -e "s/^#(crowd_connector_port):.*/\1: \"8095\"/g" $TMP_VARS
perl -i -p -e "s/^#(crowd_gid):.*/\1: ~/g" $TMP_VARS
perl -i -p -e "s/^#(crowd_hash_salt):.*/\1: \"$SALT\"/g" $TMP_VARS
perl -i -p -e "s/^#(crowd_home):.*/\1: \"\/var\/lib\/crowd\"/g" $TMP_VARS
perl -i -p -e "s/^#(crowd_pass):.*/\1: \"$PASSWD\"/g" $TMP_VARS
perl -i -p -e "s/^#(crowd_proxy_name):.*/\1: \"crowd.example.com\"/g" $TMP_VARS
perl -i -p -e "s/^#(crowd_scheme):.*/\1: \"http\"/g" $TMP_VARS
perl -i -p -e "s/^#(crowd_server_port):.*/\1: \"8020\"/g" $TMP_VARS
perl -i -p -e "s/^#(crowd_uid):.*/\1: ~/g" $TMP_VARS
perl -i -p -e "s/^#(crowd_user):.*/\1: \"crowd\"/g" $TMP_VARS
perl -i -p -e "s/^#(mysql_connector_java_dest):.*/\1: \"{{ crowd_catalina }}\/apache-tomcat\/lib\"/g" $TMP_VARS
perl -i -p -e "s/^#(mysql_connector_java_user):.*/\1: \"{{ crowd_user }}\"/g" $TMP_VARS
...
perl -i -p -e "s/^#(mysql_vhosts_id):.*/\1: \"{{ crowd_user }}\"/g" $TMP_VARS
perl -i -p -e "s/^#(mysql_vhosts_pass):.*/\1: \"{{ crowd_pass }}\"/g" $TMP_VARS
perl -i -p -e "s/^#(mysql_vhosts_user):.*/\1: \"{{ crowd_user }}\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_port):.*/\1: \"5432\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_vhosts_encoding):.*/\1: \"UTF8\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_vhosts_id):.*/\1: \"{{ crowd_user }}\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_vhosts_lc_collate):.*/\1: \"C\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_vhosts_lc_ctype):.*/\1: \"C\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_vhosts_pass):.*/\1: \"{{ crowd_pass }}\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_vhosts_template):.*/\1: \"template0\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_vhosts_user):.*/\1: \"{{ crowd_user }}\"/g" $TMP_VARS
mkdir -p inventory/crowd.aio/group_vars
mkdir -p inventory/crowd.aio/host_vars
cat $TMP_VARS >> inventory/crowd.aio/group_vars/all
