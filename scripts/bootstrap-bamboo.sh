...
# Prepare inventory/host_vars/bamboo.aio.
TMP_VARS=`mktemp`
...
perl -i -p -e "s/^#(apache2_vhosts_home):.*/\1: \"\/var\/lib\/bamboo\"/g" $TMP_VARS
...
perl -i -p -e "s/^#(apache2_vhosts_id):.*/\1: \"bamboo\"/g" $TMP_VARS
...
perl -i -p -e "s/^#(apache2_vhosts_server_name):.*/\1: \"bamboo.example.com\"/g" $TMP_VARS
perl -i -p -e "s/^#(apache2_vhosts_uid):.*/\1: ~/g" $TMP_VARS
perl -i -p -e "s/^#(apache2_vhosts_user):.*/\1: \"bamboo\"/g" $TMP_VARS
...
perl -i -p -e "s/^#(bamboo_gid):.*/\1: \"{{ apache2_vhosts_gid }}\"/g" $TMP_VARS
perl -i -p -e "s/^#(bamboo_hash_salt):.*/\1: \"{{ apache2_vhosts_hash_salt }}\"/g" $TMP_VARS
perl -i -p -e "s/^#(bamboo_home):.*/\1: \"{{ apache2_vhosts_home }}\"/g" $TMP_VARS
perl -i -p -e "s/^#(bamboo_pass):.*/\1: \"{{ apache2_vhosts_pass }}\"/g" $TMP_VARS
perl -i -p -e "s/^#(bamboo_proxy_name):.*/\1: \"{{ apache2_vhosts_server_name }}\"/g" $TMP_VARS
perl -i -p -e "s/^#(bamboo_scheme):.*/\1: \"http\"/g" $TMP_VARS
perl -i -p -e "s/^#(bamboo_server_port):.*/\1: \"8007\"/g" $TMP_VARS
perl -i -p -e "s/^#(bamboo_uid):.*/\1: \"{{ apache2_vhosts_uid }}\"/g" $TMP_VARS
perl -i -p -e "s/^#(bamboo_user):.*/\1: \"{{ apache2_vhosts_user }}\"/g" $TMP_VARS
perl -i -p -e "s/^#(mysql_connector_java_dest):.*/\1: \"\/usr\/share\/bamboo\/lib\"/g" $TMP_VARS
...
cat $TMP_VARS >> inventory/host_vars/bamboo.aio
