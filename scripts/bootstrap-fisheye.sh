...
perl -i -p -e "s/^#(apache2_vhosts_proxy_pass):.*/\1: \"\/   http:\/\/localhost:{{ fisheye_connector_port }}\/\"/g" $TMP_VARS
perl -i -p -e "s/^#(apache2_vhosts_proxy_pass_reverse):.*/\1: \"\/   http:\/\/localhost:{{ fisheye_connector_port }}\/\"/g" $TMP_VARS
...
perl -i -p -e "s/^#(apache2_vhosts_server_name):.*/\1: \"{{ fisheye_proxy_name }}\"/g" $TMP_VARS
...
perl -i -p -e "s/^#(fisheye_catalina):.*/\1: \"\/usr\/share\/fisheye\"/g" $TMP_VARS
perl -i -p -e "s/^#(fisheye_connector_port):.*/\1: \"8060\"/g" $TMP_VARS
perl -i -p -e "s/^#(fisheye_gid):.*/\1: ~/g" $TMP_VARS
perl -i -p -e "s/^#(fisheye_hash_salt):.*/\1: \"$SALT\"/g" $TMP_VARS
perl -i -p -e "s/^#(fisheye_home):.*/\1: \"\/var\/lib\/fisheye\"/g" $TMP_VARS
perl -i -p -e "s/^#(fisheye_pass):.*/\1: \"$PASSWD\"/g" $TMP_VARS
perl -i -p -e "s/^#(fisheye_proxy_name):.*/\1: \"fisheye.example.com\"/g" $TMP_VARS
perl -i -p -e "s/^#(fisheye_scheme):.*/\1: \"http\"/g" $TMP_VARS
perl -i -p -e "s/^#(fisheye_server_port):.*/\1: \"8059\"/g" $TMP_VARS
perl -i -p -e "s/^#(fisheye_uid):.*/\1: ~/g" $TMP_VARS
perl -i -p -e "s/^#(fisheye_user):.*/\1: \"fisheye\"/g" $TMP_VARS
perl -i -p -e "s/^#(mysql_connector_java_dest):.*/\1: \"{{ fisheye_catalina }}\/lib\"/g" $TMP_VARS
perl -i -p -e "s/^#(mysql_connector_java_user):.*/\1: \"{{ fisheye_user }}\"/g" $TMP_VARS
...
perl -i -p -e "s/^#(mysql_vhosts_id):.*/\1: \"{{ fisheye_user }}\"/g" $TMP_VARS
perl -i -p -e "s/^#(mysql_vhosts_pass):.*/\1: \"{{ fisheye_pass }}\"/g" $TMP_VARS
perl -i -p -e "s/^#(mysql_vhosts_user):.*/\1: \"{{ fisheye_user }}\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_port):.*/\1: \"5432\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_vhosts_encoding):.*/\1: \"UTF8\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_vhosts_id):.*/\1: \"{{ fisheye_user }}\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_vhosts_lc_collate):.*/\1: \"C\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_vhosts_lc_ctype):.*/\1: \"C\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_vhosts_pass):.*/\1: \"{{ fisheye_pass }}\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_vhosts_template):.*/\1: \"template0\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_vhosts_user):.*/\1: \"{{ fisheye_user }}\"/g" $TMP_VARS
mkdir -p inventory/fisheye.aio/group_vars
mkdir -p inventory/fisheye.aio/host_vars
cat $TMP_VARS >> inventory/fisheye.aio/group_vars/all
