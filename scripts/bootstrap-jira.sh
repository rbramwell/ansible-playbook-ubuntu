...
perl -i -p -e "s/^#(jira_catalina):.*/\1: \"\/usr\/share\/jira\"/g" $TMP_VARS
perl -i -p -e "s/^#(jira_connector_port):.*/\1: \"8080\"/g" $TMP_VARS
perl -i -p -e "s/^#(jira_gid):.*/\1: ~/g" $TMP_VARS
perl -i -p -e "s/^#(jira_hash_salt):.*/\1: \"$SALT\"/g" $TMP_VARS
perl -i -p -e "s/^#(jira_home):.*/\1: \"\/var\/lib\/jira\"/g" $TMP_VARS
...
perl -i -p -e "s/^#(jira_uid):.*/\1: ~/g" $TMP_VARS
perl -i -p -e "s/^#(jira_user):.*/\1: \"jira\"/g" $TMP_VARS
perl -i -p -e "s/^#(mysql_connector_java_dest):.*/\1: \"{{ jira_catalina }}\/lib\"/g" $TMP_VARS
perl -i -p -e "s/^#(mysql_connector_java_user):.*/\1: \"{{ jira_user }}\"/g" $TMP_VARS
...
perl -i -p -e "s/^#(mysql_vhosts_id):.*/\1: \"{{ jira_user }}\"/g" $TMP_VARS
perl -i -p -e "s/^#(mysql_vhosts_pass):.*/\1: \"{{ jira_pass }}\"/g" $TMP_VARS
perl -i -p -e "s/^#(mysql_vhosts_user):.*/\1: \"{{ jira_user }}\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_port):.*/\1: \"5432\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_vhosts_encoding):.*/\1: \"UTF8\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_vhosts_id):.*/\1: \"{{ jira_user }}\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_vhosts_lc_collate):.*/\1: \"C\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_vhosts_lc_ctype):.*/\1: \"C\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_vhosts_pass):.*/\1: \"{{ jira_pass }}\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_vhosts_template):.*/\1: \"template0\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_vhosts_user):.*/\1: \"{{ jira_user }}\"/g" $TMP_VARS
mkdir -p inventory/jira.aio/group_vars
mkdir -p inventory/jira.aio/host_vars
cat $TMP_VARS >> inventory/jira.aio/group_vars/all
