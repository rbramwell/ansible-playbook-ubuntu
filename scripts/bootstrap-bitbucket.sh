...
perl -i -p -e "s/^#(bitbucket_catalina):.*/\1: \"\/usr\/share\/bitbucket\"/g" $TMP_VARS
perl -i -p -e "s/^#(bitbucket_connector_port):.*/\1: \"7990\"/g" $TMP_VARS
perl -i -p -e "s/^#(bitbucket_gid):.*/\1: ~/g" $TMP_VARS
perl -i -p -e "s/^#(bitbucket_hash_salt):.*/\1: \"$SALT\"/g" $TMP_VARS
perl -i -p -e "s/^#(bitbucket_home):.*/\1: \"\/var\/lib\/bitbucket\"/g" $TMP_VARS
...
perl -i -p -e "s/^#(bitbucket_uid):.*/\1: ~/g" $TMP_VARS
perl -i -p -e "s/^#(bitbucket_user):.*/\1: \"bitbucket\"/g" $TMP_VARS
perl -i -p -e "s/^#(mysql_connector_java_dest):.*/\1: \"{{ bitbucket_catalina }}\/lib\"/g" $TMP_VARS
perl -i -p -e "s/^#(mysql_connector_java_user):.*/\1: \"{{ bitbucket_user }}\"/g" $TMP_VARS
...
perl -i -p -e "s/^#(mysql_vhosts_id):.*/\1: \"{{ bitbucket_user }}\"/g" $TMP_VARS
perl -i -p -e "s/^#(mysql_vhosts_pass):.*/\1: \"{{ bitbucket_pass }}\"/g" $TMP_VARS
perl -i -p -e "s/^#(mysql_vhosts_user):.*/\1: \"{{ bitbucket_user }}\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_port):.*/\1: \"5432\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_vhosts_encoding):.*/\1: \"UTF8\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_vhosts_id):.*/\1: \"{{ bitbucket_user }}\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_vhosts_lc_collate):.*/\1: \"C\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_vhosts_lc_ctype):.*/\1: \"C\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_vhosts_pass):.*/\1: \"{{ bitbucket_pass }}\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_vhosts_template):.*/\1: \"template0\"/g" $TMP_VARS
perl -i -p -e "s/^#(postgresql_vhosts_user):.*/\1: \"{{ bitbucket_user }}\"/g" $TMP_VARS
mkdir -p inventory/bitbucket.aio/group_vars
mkdir -p inventory/bitbucket.aio/host_vars
cat $TMP_VARS >> inventory/bitbucket.aio/group_vars/all
